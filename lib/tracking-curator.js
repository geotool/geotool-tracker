...
Clazz.prototype.stats = function() {
  var self = this;
  debuglog && debuglog(' - return geotool information');
  return {
...
var validateGeofenceCollection = (function(validatorType) {
  debuglog && debuglog(' - validatorType: %s', validatorType || 'ajv');
  if (validatorType == 'jsonschema') {
    return function(geofenceCollection, ctx) {
      ...
      if (result.errors.length > 0) {
        debuglog && debuglog(' - invalid geofenceCollection: %s', JSON.stringify(result));
      }
      debuglog && debuglog(' - validate(%s) -> %s', JSON.stringify(geofenceCollection), valid);
...
var validateTrackingpoint = (function(validatorType) {
  ...
  if (validatorType == 'jsonschema') {
    return function(trackingpoint, ctx) {
      ...
      var valid = (result.errors.length == 0);
      if (result.errors.length > 0) {
        debuglog && debuglog(' - invalid trackingpoint: %s', JSON.stringify(result, null, 2));
      }
      debuglog && debuglog(' - validate(%s) -> %s', JSON.stringify(trackingpoint), valid);
      return { success: valid }
    }
  } else {
    return function(trackingpoint, ctx) {
